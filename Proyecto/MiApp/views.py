...
def formulario_blogs(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if formulario.is_valid():
                formulario_limpio = formulario.cleaned_data
                blog = Blogs(titulo = formulario_limpio["titulo"], subtitulo = formulario_limpio["subtitulo"], autor = formulario_limpio["autor"], fecha_imagen = formulario_limpio["fecha_imagen"], imagen = formulario.cleaned_data["imagen"], cuerpo = formulario_limpio["cuerpo"])
                blog.save()
                return render(request, 'MiApp/index2.html')
        else:
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if usuario_form.is_valid():
                ...
                usuario.email = informacion['emsil']
